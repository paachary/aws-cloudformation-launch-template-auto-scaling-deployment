...
def _upload_children_template_files(stack):
    ...
    for file in range(0, len(templates)):
        ...
        with cfn_file as data:
            print("uploading file {}".format(templates[file]))
...
def create_stack(*stacks, **kwargs):
    ...
    if len(stacks) == 0:
        ...
        "Valid values are: \n\n" + 
            ...
            " \t\t\t\t\t\t\t which will be mapped to the ASG. \n\n" +
            "   aws-cloudformation-autoscaling-group-webapp-resources -> Creates an autoscaling group with 2 instances \n" +
            " \t\t\t\t\t\t\t running the Flask Web Application, accessed by the loadbalancer DNS. \n\n" )
        return
